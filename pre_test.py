...
def pad_graph(mol, adj_matrix, atom_features, edge_features, max_nodes):
    ...
    print(padded_adj.shape)
    print(1111111111111111111111)
    print(atom_features.shape)
    print(padded_atom_features.shape)
    print(1111111111111111111111)
    print(len(edge_features))
    print(atom_features)
    print(len(padded_edge_features))
    exit(-1)
...
class MoleculeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return adj_matrix, atom_features, edge_features, target

def collate_fn(batch):
    adj_matrices, atom_features, edge_features, targets = zip(*batch)
    ...
    return adj_matrices, atom_features, edge_features, targets
...
smiles_list = ["CCC", "CCC", "CCC", "CCCl", "CCBr"]  # Replace with your dataset
targets = [1, 1.5, 0.7, 0.8, 0.9]  # Example targets (replace with actual data)
max_nodes = 15  # Set to the desired maximum number of nodes for padding
...
#
#smiles_list = list(data.iloc[:100]['smi']           )
#targets      = list(data.iloc[:100]['s_energy(ev)']  )
#
...
hidden_dim = 4  # Hidden dimension for messages
num_layers = 5
...
optimizer = torch.optim.Adam(dmpnn.parameters(), lr=0.0001)
...
for epoch in range(num_epochs):
    for adj_matrices, atom_features, edge_features, targets in data_loader:
        optimizer.zero_grad()
...
def predict(smiles_list, model, max_nodes):
    ...
    predictions = []
    with torch.no_grad():
        for adj_matrices, atom_features, edge_features, _ in data_loader:
            outputs = model(atom_features, edge_features, adj_matrices)
            predictions.append(outputs.item())
    return predictions

# Example prediction
new_smiles_list = ["CCO", "CCO"]
